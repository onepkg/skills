...
> **此处未展示的绑定：** 本 README 涵盖了 Go 最常用的 managed-agents 流程。如果你需要某个未展示的类、方法、命名空间、字段或行为，请通过 WebFetch 从 `shared/live-sources.md` 中获取 Go SDK 仓库 **或相关文档页面**，而不要猜测。不要从 cURL 形式或其他语言的 SDK 进行推断。

> **Agent 是持久化的——创建一次，通过 ID 引用。** 保存 `agents.New` 返回的 agent ID，并将其传递给每一次后续的 `sessions.New`；不要在请求路径中调用 `agents.New`。Anthropic CLI 是从版本控制的 YAML 创建 agent 和环境的一种便捷方式——其 URL 在 `shared/live-sources.md` 中。下面的示例展示了完整的代码内创建过程；在生产环境中，创建调用应属于设置阶段，而非请求路径。
...
> ⚠️ **没有内联 agent 配置。** `Model`/`System`/`Tools` 存在于 agent 对象上，而非 session 上。始终以 `Beta.Agents.New()` 开始——session 只接受 `Agent: anthropic.BetaSessionNewParamsAgentUnion{OfString: anthropic.String(agent.ID)}`（或当你需要特定版本时，使用类型化的 `OfBetaManagedAgentsAgents` 变体）。
...
每次更新会创建新版本；agent 对象在每个版本上是不可变的。
...
> 💡 **Stream 优先：** 在发送消息 *之前*（或同时）打开 stream。stream 只传递其打开后发生的事件——先发送后打开 stream 意味着早期事件会作为一批缓冲到达。请参阅 [引导模式](../../shared/managed-agents-events.md#steering-patterns)。
...
当在会话中间重新连接时，先列出过去的事件以去重，然后监听实时事件：
...
> ℹ️ Go managed-agents 中 `user.custom_tool_result` 的绑定在此技能或应用源码示例中尚未记录。请参阅 `shared/managed-agents-events.md` 了解 wire 格式，并查看 `github.com/anthropics/anthropic-sdk-go` 仓库以获取对应的 Go params 类型。
...
> ℹ️ 列出和下载 agent 在会话期间写入的文件在此技能或应用源码示例中尚未为 Go 记录。请参阅 `shared/managed-agents-events.md` 和 `github.com/anthropics/anthropic-sdk-go` 仓库以获取 `Beta.Files.List` 和 `Beta.Files.Download` 的 Go params 类型。
...
有关创建 vault 和添加凭据，请参阅 `shared/managed-agents-tools.md` §Vaults。
