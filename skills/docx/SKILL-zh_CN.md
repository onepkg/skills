---
name: docx
description: "当用户想要创建、读取、编辑或操作 Word 文档（.docx 文件）时使用此技能。触发条件包括：任何提及 'Word doc'、'word document'、'.docx' 的表述，或要求制作带有目录、标题、页码或信头等格式的专业文档。也适用于从 .docx 文件中提取或重新组织内容、在文档中插入或替换图片、在 Word 文件中执行查找替换、处理修订或批注，或将内容转换为精美的 Word 文档。如果用户要求以 Word 或 .docx 文件形式提供 '报告'、'备忘录'、'信函'、'模板' 或类似交付物，请使用此技能。请勿用于 PDF、电子表格、Google 文档或与文档生成无关的常规编码任务。"
...
.docx 文件是一个包含 XML 文件的 ZIP 归档包。

## 快速参考

| 任务 | 方法 |
|------|----------|
| 读取/分析内容 | `pandoc` 或解包后查看原始 XML |
| 创建新文档 | 使用 `docx-js` - 参见下面的创建新文档 |
| 编辑已有文档 | 解包 → 编辑 XML → 重新打包 - 参见下面的编辑已有文档 |
...
要生成接受所有修订后的干净文档（需要 LibreOffice）：
...
**横向方向：** docx-js 会在内部交换宽/高，因此传入纵向尺寸，让它处理交换：
```javascript
size: {
...
使用 Arial 作为默认字体（通用支持）。标题保持黑色以保可读性。
...
**关键：表格需要双重宽度** - 同时在表格上设置 `columnWidths` 和在每个单元格上设置 `width`。缺少任何一个都会导致表格在某些平台上显示不正确。
    ...
    new TableRow({
      children: [
        new TableCell({
          ...
          margins: { top: 80, bottom: 80, left: 120, right: 120 }, // 单元格内边距（内部空间，不增加宽度）
...
new Paragraph({
  children: [new ImageRun({
    ...
    altText: { title: "Title", description: "Desc", name: "Name" } // 三者均必需
  })]
})
```
...
sections: [{
  properties: {
    column: {
      count: 2,          // 栏数
      space: 720,        // 栏间距，单位为 DXA（720 = 0.5 英寸）
      equalWidth: true,
      separate: true,    // 栏间竖线分隔
    },
  },
...
- **显式设置页面尺寸** - docx-js 默认使用 A4；美式文档请使用 US Letter（12240 x 15840 DXA）
...
- **始终使用 DXA 设置表格 `width`** - 绝不使用 `WidthType.PERCENTAGE`（在 Google Docs 中会出错）
- **表格需要双重宽度** - `columnWidths` 数组和单元格 `width`，两者必须匹配
- **表格宽度 = columnWidths 之和** - 使用 DXA 时，确保它们精确相加
- **始终添加单元格边距** - 使用 `margins: { top: 80, bottom: 80, left: 120, right: 120 }` 以获得可读的内边距
- **使用 `ShadingType.CLEAR`** - 表格底纹绝不使用 SOLID
- **绝不使用表格作为分隔线/横线** - 单元格有最小高度，会渲染为空白框（包括在页眉/页脚中）；应在 Paragraph 上使用 `border: { bottom: { style: BorderStyle.SINGLE, size: 6, color: "2E75B6", space: 1 } }` 代替。对于两栏页脚，使用制表位（参见制表位章节），而非表格
- **TOC 仅需要 HeadingLevel** - 标题段落上不能有自定义样式
- **覆盖内置样式** - 使用精确 ID："Heading1"、"Heading2" 等
- **包含 `outlineLevel`** - TOC 需要此项（H1 为 0，H2 为 1，依此类推）
...
python scripts/office/unpack.py document.docx unpacked/
```
提取 XML、美化打印、合并相邻 run，并将智能引号转换为 XML 实体（`&#x201C;` 等），以便在编辑时保留。使用 `--merge-runs false` 可跳过 run 合并。

### 第 2 步：编辑 XML

编辑 `unpacked/word/` 中的文件。参见下面的 XML 参考了解模式。

**使用 "Claude" 作为作者**来添加修订和批注，除非用户明确要求使用其他名称。

**直接使用 Edit 工具进行字符串替换。不要编写 Python 脚本。** 脚本会引入不必要的复杂性。Edit 工具精确显示正在替换的内容。
...
**添加批注：** 使用 `comment.py` 来处理跨多个 XML 文件的样板代码（文本必须预先进行 XML 转义）：
```bash
...
- **替换整个 `<w:r>` 元素**：添加修订时，将整个 `<w:r>...</w:r>` 块替换为作为兄弟元素的 `<w:del>...<w:ins>...`。不要将修订标签注入 run 内部。
- **保留 `<w:rPr>` 格式**：将原始 run 的 `<w:rPr>` 块复制到修订 run 中，以保持粗体、字号等。
...
**删除整个段落/列表项** - 当移除段落中的所有内容时，同时将段落标记标记为已删除，以便与下一段落合并。在 `<w:pPr><w:rPr>` 内部添加 `<w:del/>`：
```xml
<w:p>
  <w:pPr>
...
```
如果在 `<w:pPr><w:rPr>` 中没有 `<w:del/>`，接受更改后会留下一个空段落/列表项。
...
运行 `comment.py` 后（参见第 2 步），在 document.xml 中添加标记。对于回复，使用 `--parent` 标志并将标记嵌套在父级内部。
