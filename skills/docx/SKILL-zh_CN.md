---
name: docx
description: "当用户想要创建、读取、编辑或操作 Word 文档（.docx 文件）时使用此技能。触发条件包括：任何提及'Word doc'、'word document'、'.docx'，或请求生成带有格式的专业文档（如目录、标题、页码或信头）。还用于从 .docx 文件中提取或重新组织内容、在文档中插入或替换图像、在 Word 文件中执行查找和替换、处理修订跟踪或注释，或将内容转换为精美的 Word 文档。如果用户要求将'报告'、'备忘录'、'信件'、'模板'或类似交付物作为 Word 或 .docx 文件，请使用此技能。不要用于 PDF、电子表格、Google Docs 或与文档生成无关的通用编码任务。"
...
要生成一个干净的文档，接受所有修订跟踪（需要 LibreOffice）：
...
**横向方向：** docx-js 在内部交换宽度/高度，所以传递纵向尺寸并让它处理交换：
```javascript
size: {
  ...
  styles: {
    ...
    paragraphStyles: [
      ...
      { id: "Heading1", name: "Heading 1", basedOn: "Normal", next: "Normal", quickFormat: true,
        ...
        paragraph: { spacing: { before: 240, after: 240 }, outlineLevel: 0 } }, // outlineLevel 对 TOC 必需
...
**关键：表格需要双重宽度** - 在表格上设置 `columnWidths` 并在每个单元格上设置 `width`。没有两者，表格在某些平台上渲染不正确。
...
// 带 1" 边距的 US Letter：12240 - 2880 = 9360 DXA
...
new Paragraph({
  children: [new ImageRun({
    ...
    altText: { title: "Title", description: "Desc", name: "Name" } // 三个都必需
  })]
})
```
...
使用类型为 `SectionType.NEXT_COLUMN` 的新部分强制列换行。
...
- **显式设置页面大小** - docx-js 默认为 A4；对美国文档使用 US Letter（12240 x 15840 DXA）
...
- **PageBreak 必须在 Paragraph 中** - 独立创建会生成无效的 XML
- **ImageRun 需要 `type`** - 始终指定 png/jpg 等
- **始终用 DXA 设置表格 `width`** - 永远不要使用 `WidthType.PERCENTAGE`（在 Google Docs 中会破坏）
- **表格需要双重宽度** - `columnWidths` 数组和单元格 `width`，两者必须匹配
- **表格宽度 = columnWidths 的总和** - 对于 DXA，确保它们精确相加
- **始终添加单元格边距** - 使用 `margins: { top: 80, bottom: 80, left: 120, right: 120 }` 获得可读的填充
- **使用 `ShadingType.CLEAR`** - 永远不要对表格着色使用 SOLID
- **永远不要使用表格作为分隔线/规则** - 单元格有最小高度并渲染为空框（包括在页眉/页脚中）；改用 Paragraph 上的 `border: { bottom: { style: BorderStyle.SINGLE, size: 6, color: "2E75B6", space: 1 } }`。对于两列页脚，使用制表位（参见制表位部分），而不是表格
- **TOC 仅需要 HeadingLevel** - 标题段落上没有自定义样式
- **覆盖内置样式** - 使用确切的 ID："Heading1"、"Heading2" 等
- **包含 `outlineLevel`** - TOC 必需（H1 为 0，H2 为 1 等）
...
python scripts/office/unpack.py document.docx unpacked/
```
提取 XML，美化打印，合并相邻的运行，并将智能引号转换为 XML 实体（`&#x201C;` 等），以便它们在编辑时存活。使用 `--merge-runs false` 跳过运行合并。

### 步骤 2：编辑 XML

编辑 `unpacked/word/` 中的文件。参见下面的 XML 参考获取模式。

**使用"Claude"作为作者**用于修订跟踪和注释，除非用户明确要求使用其他名称。
...
python scripts/comment.py unpacked/ 1 "Reply text" --parent 0  # 回复注释 0
python scripts/comment.py unpacked/ 0 "Text" --author "Custom Author"  # 自定义作者名称
```
然后将标记添加到 document.xml（参见 XML 参考中的注释）。
...
- **替换整个 `<w:r>` 元素**：添加修订跟踪时，替换整个 `<w:r>...</w:r>` 块为 `<w:del>...<w:ins>...` 作为兄弟元素。不要在运行内注入修订跟踪标签。
...
- **空白**：在带有前导/尾随空格的 `<w:t>` 上添加 `xml:space="preserve"`
...
**删除整个段落/列表项** - 当从段落中删除所有内容时，也将段落标记标记为已删除，以便它与下一个段落合并。在 `<w:pPr><w:rPr>` 中添加 `<w:del/>`：
```xml
<w:p>
  <w:pPr>
...
```
如果没有 `<w:pPr><w:rPr>` 中的 `<w:del/>`，接受更改后会留下空的段落/列表项。
...
运行 `comment.py` 后（参见步骤 2），将标记添加到 document.xml。对于回复，使用 `--parent` 标志并将标记嵌套在父级内。

**关键：`<w:commentRangeStart>` 和 `<w:commentRangeEnd>` 是 `<w:r>` 的兄弟元素，永远不在 `<w:r>` 内。**

```xml
<!-- 注释标记是 w:p 的直接子元素，永远不在 w:r 内 -->
