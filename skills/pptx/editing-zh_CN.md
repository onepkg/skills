...
2. **规划幻灯片映射**：对于每个内容部分，选择一个模板幻灯片。
   ...
   ⚠️ **使用多样化的布局** — 单调的演示文稿是常见的失败模式。不要默认为基本的标题 + 项目符号幻灯片。积极寻找：
   - 多列布局（2 列、3 列）
   - 图像 + 文本组合
   - 全出血图像带文本叠加
   - 引用或标注幻灯片
...
打印要添加到 `<p:sldIdLst>` 所需位置的 `<p:sldId>`。
...
创建带有幻灯片文件名作为标签的 `thumbnails.jpg`。默认 3 列，每个网格最多 12 个。

**仅用于模板分析**（选择布局）。对于视觉 QA，使用 `soffice` + `pdftoppm` 创建全分辨率的单独幻灯片图像 — 参见 SKILL.md。
...
**添加**：使用 `add_slide.py`。永远不要手动复制幻灯片文件 — 脚本处理手动复制会错过的注释引用、Content_Types.xml 和关系 ID。

---

## 编辑内容

**子代理：** 如果可用，在此处使用它们（在完成步骤 4 之后）。每个幻灯片都是一个单独的 XML 文件，因此子代理可以并行编辑。在给子代理的提示中，包括：
- 要编辑的幻灯片文件路径
- **"对所有更改使用 Edit 工具"**
- 下面的格式规则和常见陷阱
...
**使用 Edit 工具，而不是 sed 或 Python 脚本。** Edit 工具强制明确要替换什么以及在哪里替换，从而提高可靠性。

### 格式规则

- **粗体所有标题、副标题和内联标签**：在 `<a:rPr>` 上使用 `b="1"`。这包括：
  - 幻灯片标题
  - 幻灯片内的部分标题
...
**模板槽位 ≠ 源项目**：如果模板有 4 个团队成员但源有 3 个用户，删除第 4 个成员的整个组（图像 + 文本框），而不仅仅是文本。
...
从原始段落复制 `<a:pPr>` 以保持行距。在标题上使用 `b="1"`。

### 智能引号

由 unpack/pack 自动处理。但 Edit 工具将智能引号转换为 ASCII。
