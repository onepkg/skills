...
slide.addText("Title", {
  ...
  margin: 0  // 当将文本与其他元素（如形状或图标）对齐时使用 0
});
```
...
slide.addText("• First item", { ... });  // 会创建双项目符号
...
// ⚠️ 不要与矩形强调叠加层配对 — 它们不会覆盖圆角。改用 RECTANGLE。
...
| `color` | string | 6 位十六进制（例如 `"000000"`） | 没有 `#` 前缀，没有 8 位十六进制 — 参见常见陷阱 |
| `blur` | number | 0-100 pt | |
| `offset` | number | 0-200 pt | **必须为非负数** — 负值会损坏文件 |
| `angle` | number | 0-359 度 | 阴影投射方向（135 = 右下，270 = 向上） |
| `opacity` | number | 0.0-1.0 | 使用此属性进行透明度设置，永远不要在颜色字符串中编码 |

要向上投射阴影（例如在页脚栏上），使用 `angle: 270` 和正偏移 — **不要**使用负偏移。
...
2. **永远不要在十六进制颜色字符串中编码不透明度** - 8 位颜色（例如 `"00000020"`）会损坏文件。改用 `opacity` 属性。
   ```javascript
   shadow: { type: "outer", blur: 6, offset: 2, color: "00000020" }          // ❌ 损坏文件
   shadow: { type: "outer", blur: 6, offset: 2, color: "000000", opacity: 0.12 }  // ✅ 正确
   ```

3. **使用 `bullet: true`** - 永远不要使用 unicode 符号如"•"（会创建双项目符号）

4. **在数组项或文本运行之间使用 `breakLine: true`**

5. **避免在带有项目符号时使用 `lineSpacing`** - 会导致过大的间隙；改用 `paraSpaceAfter`

6. **每个演示文稿需要新实例** - 不要重用 `pptxgen()` 对象

7. **永远不要在调用之间重用选项对象** - PptxGenJS 会就地变异对象（例如将阴影值转换为 EMU）。在多个调用之间共享一个对象会损坏第二个形状。
   ```javascript
   const shadow = { type: "outer", blur: 6, offset: 2, color: "000000", opacity: 0.15 };
   slide.addShape(pres.shapes.RECTANGLE, { shadow, ... });  // ❌ 第二次调用获取已转换的值
