...
// 字符间距（使用 charSpacing，不要使用 letterSpacing，后者会被静默忽略）
...
slide.addText("Title", {
  ...
  margin: 0  // 当文本需要与形状或图标等其他元素对齐时设为 0
});
```

**提示：** 文本框默认有内部边距。当文本需要与相同 x 坐标的形状、线条或图标精确对齐时，请设置 `margin: 0`。
...
| `color` | string | 6 字符十六进制（如 `"000000"`） | 不加 `#` 前缀，不使用 8 字符十六进制——参见常见陷阱 |
| `blur` | number | 0-100 pt | |
| `offset` | number | 0-200 pt | **必须为非负数**——负值会损坏文件 |
| `angle` | number | 0-359 度 | 阴影投射的方向（135 = 右下，270 = 向上） |
| `opacity` | number | 0.0-1.0 | 使用此属性设置透明度，切勿编码到颜色字符串中 |

若要向上投射阴影（例如在页脚栏上），请使用 `angle: 270` 加正数偏移——**不要**使用负数偏移。

**注意：** 渐变填充原生不支持。请改用渐变图片作为背景。
...
- **SVG**：在现代化 PowerPoint/Microsoft 365 中可用
...
**注意：** 使用 256 或更大的尺寸以获得清晰的图标。size 参数控制栅格化分辨率，而非幻灯片上的显示尺寸（显示尺寸由 `w` 和 `h` 的英寸值决定）。
...
2. **切勿将不透明度编码到十六进制颜色字符串中**——8 字符颜色（如 `"00000020"`）会损坏文件。请改用 `opacity` 属性。
   ```javascript
...
5. **避免在项目符号上使用 `lineSpacing`**——会导致间距过大；请改用 `paraSpaceAfter`

6. **每个演示文稿需要新的实例**——不要复用 `pptxgen()` 对象

7. **切勿在多次调用之间复用 option 对象**——PptxGenJS 会就地修改对象（例如将阴影值转换为 EMU）。在多次调用之间共享同一个对象会损坏第二个形状。
   ```javascript
   ...
   slide.addShape(pres.shapes.RECTANGLE, { shadow: makeShadow(), ... });  // ✅ 每次都使用新对象
